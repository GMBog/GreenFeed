...
VR <- read_csv(file_path, col_names = FALSE, col_types = cols(X1 = col_character())) %>% dplyr::select(c(-12:-16)) %>% dplyr::mutate(X11 = X10)
...
Bins <- seq(1,32); VR <- VR[VR$X3 %in% Bins, ]
...
Raw_data <- VR %>% dplyr::mutate(Date = Date, Type = "FEED") %>% dplyr::relocate(Date, Type, .before = X1)
...
VR$X4 <- format(VR$X4, "%H:%M:%S"); VR$X5 <- format(VR$X5, "%H:%M:%S")
...
ggplot(Final_data, aes(x = as.character(Visible_ID), y = FedKg, fill = Feed)) +
  geom_col(width = 0.3) +
  theme_grey() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1.05, size = 8, family = "Times New Roman"),
